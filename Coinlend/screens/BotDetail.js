...
import { Image, View, Text, FlatList, ActivityIndicator, SafeAreaView, TouchableOpacity } from 'react-native';
...
class BotDetailListItem extends React.PureComponent {
	...
	render() {
		...
		return (
			<TouchableOpacity onPress={() => this._onPress(this.props.item.id)}>
	...
	class BotDetailFlatList extends React.Component {
		...
		_renderHeader = () => {
			...
			_renderItem = ({ item }) => (
				<BotDetailListItem item={item} navigation={this.props.navigation} containerStyle={{ borderBottomWidth: 0 }}/>
			);
		...
		class BotDetail extends React.Component {
			...
			render() {
				return (
					<BotDetailFlatList navigation={this.props.navigation}/>
				);
			}
		}
